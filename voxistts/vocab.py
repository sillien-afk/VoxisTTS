...
voxistts.vocab
=============
Kokoro-82M 的 IPA 音素契约，直接来自模型 config.json 的 `vocab` 字段。
VoxisModel.forward 用 self.vocab.get(p) 把每个音素字符映射成 id；
不在本表中的字符会被静默丢弃，因此 G2P 输出必须全部落在本表内。

来源：E:/llm_models/kokoro-82m/config.json -> config["vocab"]
"""

VOCAB = {
    ";": 1, ":": 2, ",": 3, ".": 4, "!": 5, "?": 6, "—": 9, "…": 10,
    "\"": 11, "(": 12, ")": 13, "“": 14, "”": 15, " ": 16,
    "\u0303": 17,            # 组合波浪号（普通话声调标记，见 phoneme_data.py）
...
# 集合形式，便于 G2P 输出校验
VALID_PHONEMES = frozenset(VOCAB.keys())

# 标点符号（直接透传，无需 G2P）
PUNCTUATION = frozenset(";:,.!?—…\"()“” ")

# 模型可接受的文本最大音素长度（VoxisModel 约束 input_ids+2 <= context_length=512）
MAX_PHONEME_LEN = 510
